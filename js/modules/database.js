...
// Versão Corrigida - Sem duplicação de funções
// ============================================
...
const Database = (function() {
    ...
    // ===== Sincronizar Professores com Usuários =====
    ...
    function getMonitoresPorTurma(turma) {
        ...
        if (turma) return db.monitores[turma] || [];
        return db.monitores;
    }
    ...
    function getReservasPorData(data) {
        const labs = ['Lab Informática', 'Lab Hardware', 'Lab Multimídia'];
        const reservas = {};
        
        for (const lab of labs) {
            reservas[lab] = {};
            // Usar SLOTS do Reservations se disponível
            const slots = (typeof Reservations !== 'undefined' && Reservations.SLOTS) 
                ? Reservations.SLOTS 
                : [
            ...
            for (const slot of slots) {
                const key = `res-${lab}-${data}-${slot}`;
                const reserva = localStorage.getItem(key);
                reservas[lab][slot] = reserva ? JSON.parse(reserva) : { p: '', t: '' };
            }
        }
        return reservas;
    }
